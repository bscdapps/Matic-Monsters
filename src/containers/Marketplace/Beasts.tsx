...
import { getOnMarketplace, getBeastToken, getBaseUrl, getMarketplaceMOONLIGHTAllowance, setMarketplaceMOONLIGHTApprove, cancelMarketplace, buyToken, getMarketItem } from '../../hooks/contractFunction';
import { useBeast, useMarketplace, useMOONLIGHT, useWeb3 } from '../../hooks/useContract';
...
const Beasts = () => {
	...
	const [sortMLT, setSortMLT] = React.useState(false);
	...
	const handleBuy = async (id: number) => {
		setActionLoading(true);
		const allowance = await getMarketplaceMOONLIGHTAllowance(web3, MOONLIGHTContract, account);
		try {
			if (allowance === '0') {
				await setMarketplaceMOONLIGHTApprove(web3, MOONLIGHTContract, account);
			}
	...
	const handleSortMLT = (value: boolean) => {
		setSortMLT(value);
		handleSort('MLT');
	}
					...
					<Grid item xs={12} md={6} xl={4}>
						<FormControl component="fieldset" sx={{ width: '90%' }}>
							...
							<ButtonGroup variant="outlined" color="primary" sx={{ pt: 1 }}>
								<Button variant={!sortMLT ? "contained" : "outlined"} onClick={() => { handleSortMLT(!sortMLT) }}>{getTranslation('lowest')}</Button>
								<Button variant={sortMLT ? "contained" : "outlined"} onClick={() => { handleSortMLT(!sortMLT) }}>{getTranslation('highest')}</Button>
							</ButtonGroup>
						</FormControl>
					</Grid>
					<Grid
